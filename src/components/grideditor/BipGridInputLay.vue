<template>
    <div>
        <template v-if="cell.editType === 1">
            <bip-grid-list-editor :cds="cds" :cell="cell" :index="index" :model="model"></bip-grid-list-editor>
        </template>
        <template v-else>
            ...
            <template v-else-if="cell.type == 91 || cell.type==93 || cell.assType=='C_DATE'">
                <bip-grid-date-editor :cds="cds" :cell="cell" :index="index" :model="model"></bip-grid-date-editor>
            </template>
            <template v-else-if="cell.type == 3">
                <bip-grid-number-editor :cds="cds" :cell="cell" :index="index" :model="model"></bip-grid-number-editor>
            </template>
            <template v-else-if="cell.assist&&cell.editName!='COPY'">
                <bip-grid-aid-editor :cds="cds" :cell="cell" :index="index" :model="model"></bip-grid-aid-editor>
            </template>
            <template v-else>
                <bip-grid-comm-editor :cds="cds" :cell="cell" :index="index" :model="model"></bip-grid-comm-editor>
            </template>
        </template>
    </div>
</template>
