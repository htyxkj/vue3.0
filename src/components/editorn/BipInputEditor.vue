<template>
    <el-col :span="span" :xs="24" :sm="24" :md="span">
        <template v-if="!bgrid">
            <template v-if="cell.ccVerCell>1 && (cds.ccells.widthCell==cell.ccHorCell)">
                ...
                <el-input :placeholder="cell.placeholder" style="margin-bottom:5px" type="textarea" :rows="cell.ccVerCell" :maxlength="cell.ccLeng" :style="cell.desc?'width: calc(100% - 29px);':''" v-model="model1" size="medium" :clearable="clearable" :disabled="(cell.attr&0x40)>0" @change="dataChange" @focus="focus"></el-input>
            ...
            <template v-else>
                <el-form-item :label="cell.labelString" class="bip-input-item" :class="cell.labelString.indexOf('.,') == 0 ?'bip-input-item-notitle':''" :required="cell.isReq">
                    ...
                    <el-input :placeholder="cell.placeholder" :type="cell.ccVerCell>1?'textarea':'text'" :rows="cell.ccVerCell" :maxlength="cell.ccLeng" :style="cell.desc?'width: calc(100% - 29px);':''" v-model="model1" size="medium" :clearable="clearable" :disabled="(cell.attr&0x40)>0" @change="dataChange" @focus="focus"></el-input>
                    <template v-if="cell.desc">
                        <span style="position:relative;line-height:32px;width:29px;padding: 5px 0px 5px 5px;">
                            <el-tooltip class="item" effect="dark" :content="cell.desc" placement="top">
                                <i class="iconfont icon-bip-bangzhu" style="font-size:14px;"></i>
