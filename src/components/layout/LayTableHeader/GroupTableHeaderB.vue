        ...
        <template v-else-if="item.child.length>0">
            <vxe-table-column :title="item.title" header-align="center">
                <template v-for="(child,index) in item.child">
                    <group-table-header-a :item="child" :key="index" :widths="widths" :cds="cds" :fixedColumn="fixedColumn"></group-table-header-a>
                </template>
            </vxe-table-column>
        </template>
        <template v-else-if="item.child.length == 0">
...
export default class GroupTableHeaderB extends Vue{
    name:string = "GroupTableHeaderB"
    @Prop() item?:any
    @Prop() cds!: CDataSet;
    @Prop() widths?:any
