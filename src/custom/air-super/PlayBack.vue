<template>
  <div>
        ...
        <el-container class="padding0" :style="style">
          <el-container class="padding0 mapMain">
            ...
            <el-main class="padding0" style="overflow: hidden;position: relative;">
                <div class="nav-tools">
                    <!-- 搜索 -->
                   <el-button icon="el-icon-search"  circle  @click="showOperaDia =!showOperaDia"></el-button>
                   <!-- 播放 -->
    ...
    <el-dialog title="查找飞防任务" :visible.sync="showOperaDia" width="50%"  class="bip-query">
...
export default class OperatingArea extends Vue {
  ...
  @Provide() operaTjCell: CDataSet = new CDataSet(""); //飞防任务对象(查询条件)
  ...
  async created() {
    ...
    this.operaTjCell = await this.getCell("B07ATJ");
   ...
   */
  async getOpera() {
        let sid = this.operaTjCell.currRecord.data.sid;
        let bgtime = this.operaTjCell.currRecord.data.bgtime;
        let edtime = this.operaTjCell.currRecord.data.edtime;
        if(sid=="" || sid == null){
             this.$notify.error("任务编码不能为空");
             return
        }
        if(bgtime=="" || bgtime==null){
        ...
        let cont = "tkid  = " + "'" + sid + "'" ;
        // + " and " +"speedtime >=" + "'"+ bgtime+"'" + " and " + "speedtime<=" + "'"+edtime+"'"
        qe.cont = cont;
        let cc = await tools.getBipInsAidInfo("CORRD",210,qe)
        this.showOperaDia = false;
        if(cc.data.id ==0 ){
            console.log(cc.data.data.data.values);
        this.tableData = cc.data.data.data.values;
